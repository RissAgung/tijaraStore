...
@section('content')
        ...
        <div class="flex flex-row px-4 md:px-8 justify-between gap-0 mt-0 md:flex-row py-2">
            ...
            <div class=" flex flex-row justify-between w-full">
                <div class="flex flex-row px-2 gap-4 mb-2 poppins-medium cursor-default relative">
                    <a href="/diskon">
                        <p class="text-[#8F8F8F]  px-1 py-3 hover:bg-gray-200 rounded-md">Diskon</p>
                    </a>
                    <p class="px-1 py-3">Voucher</p>
                    <div class="h-[4px] w-20 bg-primary absolute bottom-0 translate-x-[76px]">
    ...
    <div
        ...
        <div class="flex-row flex px-4 rounded-md gap-2 items-center order-1 w-full md:w-[310px]">
            ...
            {{-- @csrf --}}
            <input id="keyword" value="{{ Request::segment(2) == 'search' ? Request::segment(3) : '' }}" name="find"
                class=" py-2 px-2 w-full flex-grow outline-none" type="text" placeholder="Masukkan kode voucher">

        </div>
        <div class="flex overflow-x-scroll gap-3 scrollbar-hide flex-row h-full justify-end w-fit order-3 bg-red-400">
            {{-- isi --}}
            <a href="/voucher" id="btn_reset"
    ...
    <div class="h-[70vh] flex flex-col px-6 py-4">
        @if (count($voucher->items()) != 0)
                ...
                <table class=" w-full border-separate border-spacing-y-4">
                    <thead>
                        <tr>
                            ...
                            <th class="tracking-wide text-center text-sm poppins-regular">Kode Voucher</th>
                            <th class="tracking-wide text-center text-sm poppins-regular">Nama Voucher</th>
                    ...
                    <tbody>
                        <form id="form_delete" action="/voucher/delete_selected" method="post">
                            @csrf
                            @foreach ($voucher as $item)
                                <tr class="bg-white border-2 ">
                                    ...
                                    <td class="tracking-wide text-center p-3">{{ $item->kode_voucher }}</td>
                                    <td class="tracking-wide text-center p-3">{{ $item->nama_voucher }}</td>
                                    <td class="tracking-wide text-center p-3">{{ $item->kategori }}</td>
                                    <td class="tracking-wide text-center p-3">
                                        {{ $item->kategori == 'persen' ? $item->nominal . '%' : 'Rp. ' . str_replace(',', '.', number_format($item->nominal)) }}
                                    </td>
                                    <td class="tracking-wide text-center p-3">
                                        <div class="flex flex-row gap-2 justify-center">
                                            <div onclick="showModalUbah({{ $item }})"
                                            ...
                                            <div onclick="hapusData('/voucher/delete/{{ $item->kode_voucher }}?token={{ csrf_token() }}')"
            ...
            <div class="mt-4 flex flex-col justify-center md:flex-row md:justify-between gap-2 py-2 items-center">
                {{ $voucher->onEachSide(2)->links('vendor.pagination.CustomPagination') }}
            </div>
        @else
...
@section('otherjs')
    @include('voucher.controller')
    @if ($errors->any())
        @if (session('update'))
            <script>
                showModalUbah();
                selected_jenis_update = '{{ old('jenis_voucher_update') }}'
            </script>
        @elseif(session('tambah'))
            <script>
                showModalTambah()
                selected_jenis = '{{ old('jenis_voucher') }}'
            </script>
        @endif
    @endif
@endsection
