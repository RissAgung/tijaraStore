    ...
    <div
        ...
        <form action="{{ route('cari_restock') }}" method="get">
            <div class="flex px-4">
                ...
                <input type="search" class="flex mx-2 " placeholder="nama supplier / kode transaksi" name="search"
                    id="field_search" value="{{ request('search') }}">
            </div>
        </form>
                    ...
                    <tbody>
                        @foreach ($combinedata['data'] as $item)
                            <tr class="bg-white border-2 outline outline-[1px] outline-[#DCDADA] rounded-md">
                                <td class="text-center p-3">{{ $item->kode_pengeluaran }}</td>
                                ...
                                <td class="text-center p-3">{{ $item->jumlah }}</td>
