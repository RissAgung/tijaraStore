...
<form id="form_operasional" action="{{ url('pengeluaran/operasional.store') }}" method="post">
    @csrf
    <div id="konten_modal_add_operasional"
        class="fixed w-[95%] md:w-[500px] flex flex-col justify-between p-4 min-[374px]:p-5 md:p-7 z-[101] left-[50%] top-[50%] -translate-y-[50%] -translate-x-[50%] rounded-md drop-shadow-lg scale-0 transition ease-linear duration-200 bg-white">

        {{-- top --}}
        <div
            class="flex justify-between w-full h-10 min-[374px]:h-11 lg:text-[18px] md:text-[17px] text-[14px] pt-1 min-[374px]:mb-3 md:mb-8">
            <h1 class="poppins-medium leading-none">Tambah Operasional</h1>
        ...
        {{-- content --}}
        <div class="w-full flex flex-col overflow-auto gap-2 lg:text-[16px] md:text-[15px] text-[12px]">

            {{-- nama Produk --}}
            <div class="block">
                <p class="poppins-medium text-[#535353] mb-1 min-[374px]:mb-2">Keterangan</p>
                ...
                <textarea name="keterangan" id="keterangan" rows="10"
                    class="text-[#535353] placeholder:text-[#b3b3b3] w-full outline-none border-[2px] border-[#DDDDDD] rounded-md p-2" placeholder="Keterangan pengeluaran operasional.....">{{ old('keterangan') }}</textarea>
            </div>

            {{-- nama Produk --}}
            <div class="block">
                <p class="poppins-medium text-[#535353] mb-1 min-[374px]:mb-2">Total Biaya</p>
                <input type="text" id="total" maxlength="19" name="total" value="{{ old('total') }}"
                    class="text-[#535353] outline-none w-full h-10 min-[374px]:h-11 md:h-14 border-[2px] border-[#DDDDDD] rounded-md pl-2">
            </div>
