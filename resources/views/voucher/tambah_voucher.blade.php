...
<form id="form_voucher" action="/voucher/add" method="post">
    @csrf
    <div id="konten_modal_tambah"
        ...
        <div class="flex flex-row justify-between">
            <div class="flex justify-start px-4 md:px-8 py-6">
                <p id="title_modal">Tambah Voucher</p>
            </div>
        ...
        <div class="w-full flex-grow overflow-y-auto flex flex-col px-4 md:px-8 gap-4">
            ...
            <div id="container-nominal" class="flex flex-col gap-2 w-full mt-8">
                ...
                <div class="flex flex-row gap-2 w-full">
                    <input maxlength="13" id="txt_kode_voucher" name="txt_kode_voucher" type="text" placeholder=""
                        class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex"
                        value="{{ old('txt_kode_voucher') }}">
                </div>
                @error('txt_kode_voucher')
            ...
            <div id="container-nominal" class="flex flex-col gap-2 w-full mt-0">
                ...
                <div class="flex flex-row gap-2 w-full">
                    <input maxlength="20" id="txt_nama" name="txt_nama" type="text" placeholder=""
                        class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex"
                        value="{{ old('txt_nama') }}">
                </div>
                @error('txt_nama')
            ...
            <div class="flex flex-col gap-2 w-full">
                ...
                <div class="flex flex-row gap-2 w-full">
                    <div class="relative w-full">
                        ...
                        <select maxlength="15" id="jenis_voucher" name="jenis_voucher"
                            class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex appearance-none"
                            value="">
                            <option {{ old('jenis_voucher') == 'persen' ? 'selected' : '' }} value="persen">Persen
                            </option>
                            <option {{ old('jenis_voucher') == 'nominal' ? 'selected' : '' }} value="nominal">Nominal
                            </option>
                        </select>
                    </div>
                </div>
            ...
            <div id="container-nominal" class="flex flex-col gap-2 w-full">
                <span>Nominal</span>
                <div class="flex flex-row gap-2 w-full">
                    <input maxlength="{{ old('jenis_voucher') == 'nominal' ? '13' : '2' }}" id="txt_nominal" name="txt_nominal" type="text" placeholder=""
                        class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex"
                        value="{{ old('txt_nominal') }}">
                </div>
                @error('txt_nominal')
                    <p class="label-error text-sm text-red-700">{{ $message }}</p>
                @enderror
