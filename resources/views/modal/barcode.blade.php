...
<div id="bg_modal_barcode" class="z-[104] fixed bg-black w-full h-full opacity-0 transition pointer-events-none"></div>


{{-- TODO: INI KONTEN MODAL --}}
<div id="konten_modal_barcode"
    ...
    <div class="flex flex-row justify-between">
        <div class="flex justify-start px-4 md:px-8 py-6">
            <p id="title_modal">Barcode Product</p>
        </div>
        <div class="flex flex-row justify-start px-4 md:px-8 py-6 gap-3">
            <div onclick="closeModalBarcode()" class="cursor-pointer">
    ...
    {{-- ? end header ? --}}
    {{-- ? start isi modal ? --}}
    <div class="w-full flex-grow overflow-y-auto mt-4 md:mt-0 flex flex-col md:flex-row md:flex-wrap md:gap-4 md:pt-8 px-4 md:px-8">
        {{-- ? Isinya --}}
        <div class="justify-center flex h-32 w-full">
            <img class="w-full" id="img_barcode" src="" alt="barcode" /><br><br>
        </div>
    </div>
    ...
    <div class="w-full px-4 md:px-8 py-4">
        <button type="button" id="btn_submit" class="w-full bg-[#FFB015] flex justify-center py-4 rounded-md">
            <span id="button_submit" class="text-xs poppins-medium">OK</span>
        </button>
    </div>
    {{-- ? end footer ? --}}
</div>
