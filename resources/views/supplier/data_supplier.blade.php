...
@section('modal')
    @include('modal.add_supplier')
    @include('modal.update_supplier')
@endsection

@section('content')

    <div class=" h-fit bg-white border-b-2">
        <div class=" flex flex-col px-4 md:px-12 justify-between gap-4 md:flex-row py-4">
            ...
            <div class="flex-row w-full max-w-md flex px-4 rounded-md gap-2 items-center">
                ...
                <form class="w-full" action="/supplier/search" method="GET">
                    {{-- @csrf --}}
            ...
            <div class=" flex flex-row gap-2 justify-between">
                <div class="flex flex-row gap-2">
                    <div id="btn_hapus"
                        class="bg-[#000000] w-fit px-4 rounded-md flex gap-2 justify-center items-center drop-shadow-sm py-2 cursor-pointer">
                        ...
                        <p class="text-white poppins-regular  lg:flex">Hapus</p>
                    </div>
                    <div id="btn_tambah"
                        class="bg-[#FFB015] hover:bg-[#e7a013] w-fit px-4 gap-2 rounded-md items-center flex justify-center drop-shadow-sm cursor-pointer">
                        ...
                        <p class="text-black poppins-regular  lg:flex">Tambah</p>
                    </div>
                </div>
                <a href="/supplier" id=""
    ...
    <div class="h-[80vh] flex flex-col px-6 py-4">
        @if (count($sup->items()) != 0)
                ...
                <table class=" w-full border-separate border-spacing-y-4">
                    <thead>
                        <tr>
                            ...
                            <th class="tracking-wide text-center text-sm poppins-regular">Nama Supplier</th>
                            <th class="tracking-wide text-center text-sm poppins-regular">Keterangan</th>
                            <th class="tracking-wide text-center text-sm poppins-regular">Nomor Telepon</th>
                            <th class="tracking-wide text-center text-sm poppins-regular">Alamat</th>
                            <th class="tracking-wide text-center text-sm poppins-regular">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <form id="form_delete" action="/supplier/delete_selected" method="POST">
                            @csrf
                            @foreach ($sup as $s)
                                <tr class="bg-white border-2 ">
                                    ...
                                    <td class="tracking-wide text-center p-3">{{ $s->nama_supplier }}</td>
                                    <td class="tracking-wide text-center p-3">{{ $s->keterangan_sup }}</td>
                                    <td class="tracking-wide text-center p-3">{{ $s->no_hp_supplier }}</td>
                                    <td class="tracking-wide text-center p-3">{{ $s->alamat }} </td>
                                    <td class="tracking-wide text-center p-3">
                                        <div class="flex flex-row gap-2 justify-center">
                                            ...
                                            <div onclick="hapusData('/supplier/delete/{{ $s->kode_supplier }}?token={{ csrf_token() }}')"
            ...
            <div class="mt-4 flex flex-col justify-center md:flex-row md:justify-between gap-2 py-2 items-center">
                {{ $sup->onEachSide(2)->links('vendor.pagination.CustomPagination') }}
            </div>
        @else
...
@section('otherjs')
    <script src="{{ asset('js/controllers/supplier.js') }}"></script>
    @if ($errors->any())
        @if (session('update'))
            <script>
                showModalEdit();
