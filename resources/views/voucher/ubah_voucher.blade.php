...
<div id="bg_modal_update" class="fixed bg-black w-full h-full opacity-0 transition pointer-events-none"></div>


{{-- TODO: INI KONTEN MODAL --}}
<form id="form_voucher_update" action="/voucher/update" method="post">
    @csrf
    <div id="konten_modal_update"
        ...
        <div class="flex flex-row justify-between">
            <div class="flex justify-start px-4 md:px-8 py-6">
                <p id="title_modal">Ubah Voucher</p>
            </div>
        ...
        <div class="w-full flex-grow overflow-y-auto flex flex-col px-4 md:px-8 gap-4">
            ...
            <div id="container-nominal" class="flex flex-col gap-2 w-full mt-8">
                <span>Kode Voucher</span>
                <div class="flex flex-row gap-2 w-full">
                    <input maxlength="13" id="txt_kode_voucher_update" name="txt_kode_voucher_update" type="text" readonly placeholder=""
                        class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex bg-gray-100"
                        value="{{ old('txt_kode_voucher_update') }}">
                </div>
                @error('txt_kode_voucher_update')
                    <p class="label-error text-sm text-red-700">{{ $message }}</p>
                @enderror
            </div>

            <div id="container-nominal" class="flex flex-col gap-2 w-full mt-0">
                <span>Nama Voucher</span>
                <div class="flex flex-row gap-2 w-full">
                    <input maxlength="20" id="txt_nama_update" name="txt_nama_update" type="text" placeholder=""
                        class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex"
                        value="{{ old('txt_nama_update') }}">
                </div>
            ...
            <div class="flex flex-col gap-2 w-full">
                ...
                <div class="flex flex-row gap-2 w-full">
                    <div class="relative w-full">
                        ...
                        <select maxlength="15" id="jenis_voucher_update" name="jenis_voucher_update"
                            class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex appearance-none"
                            value="">
                            <option {{ old('jenis_voucher_update') == 'persen' ? 'selected' : '' }} value="persen">Persen
                            </option>
                            <option {{ old('jenis_voucher_update') == 'nominal' ? 'selected' : '' }} value="nominal">Nominal
                            </option>
                        </select>
                    </div>
                </div>
                @error('jenis_voucher_update')
                    <p class="label-error text-sm text-red-700">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div id="container-nominal" class="flex flex-col gap-2 w-full">
                <span>Nominal</span>
                <div class="flex flex-row gap-2 w-full">
                    <input maxlength="{{ old('jenis_voucher_update') == 'nominal' ? '13' : '2' }}" id="txt_nominal_update" name="txt_nominal_update" type="text" placeholder=""
                        class="border-2 rounded-md py-2 px-4 w-full outline-none flex-grow flex"
                        value="{{ old('txt_nominal_update') }}">
                </div>
        ...
        <div class="w-full px-4 md:px-8 py-4">
            <button type="button" id="btn_submit_update" class="w-full bg-[#FFB015] flex justify-center py-4 rounded-md">
                <span id="" class="text-xs poppins-medium">Ubah Data</span>
            </button>
        </div>
        {{-- ? end footer ? --}}
    </div>
