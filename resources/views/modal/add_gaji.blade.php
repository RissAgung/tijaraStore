...
<form id="form_gaji" action="/salary/add" method="POST">
    @csrf
    <div id="konten_modal_add"
        ...
        <div
            ...
            <h1 class="poppins-medium leading-none">Gaji Pokok</h1>
        ...
        {{-- <!-- konten modal -->
          ...
          <div class="w-full flex-grow overflow-auto lg:text-[16px] md:text-[15px] text-[12px]">
            ...
            {{-- nama Produk --}}
            <div class="block">
                <Label class="ml-2 text-sm">Admin</Label>
                <input id="admin" name="admin" type="text" placeholder=""
                    class="border-2 w-full rounded-md py-2 px-4 mt-1 outline-none" value="{{ old('admin') }}">
                @error('admin')
            ...
            <div class="flex flex-col mt-2 min-[374px]:mt-4">
                <Label class="ml-2 text-sm">Kasir</Label>
                <input id="kasir" name="kasir" type="text" placeholder=""
                    class="border-2 rounded-md py-2 px-4 mt-1 outline-none" value="{{ old('kasir') }}">
                @error('kasir')
                    <p class="label-error text-sm text-red-700">{{ $message }}</p>
                @enderror
            </div>
            ...
            {{-- Kontak --}}
            <div class="flex flex-col mt-2 min-[374px]:mt-4">
                <Label class="ml-2 text-sm">Pegawai</Label>
                <input id="pegawai" name="pegawai" type="text" placeholder=""
                    class="border-2 rounded-md py-2 px-4 mt-1 outline-none" value="{{ old('pegawai') }}">
                @error('pegawai')
                    <p class="label-error text-sm text-red-700">{{ $message }}</p>
                @enderror
            </div>          
